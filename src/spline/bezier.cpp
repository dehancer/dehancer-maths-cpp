...
namespace dehancer {
    namespace spline {
        ...
        static math::float4x4 matrix = { { 1,  0,  0, 0},
                                         {-3,  3,  0, 0},
                                         { 3, -6,  3, 0},
                                         {-1,  3, -3, 1}
        };

        float Bezier::value(float x) const {
        ...
        size_t Bezier::minimum_controls() const {
            return 4;
        }

        void Bezier::spline(std::vector<math::float2>& sp, const math::float4& x, const math::float4& y, size_t np) {
        ...
        void Bezier::make_nps(std::vector<size_t> &nps) {
